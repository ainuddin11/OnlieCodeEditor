...
const pythontDefault = `# Python 3 program for recursive binary search.
...
const Landing = () => {
  const [code, setCode] = useState(pythontDefault);
  ...
  useEffect(() => {
    defineTheme("pastels-on-dark").then((_) =>
      setTheme({ value: "pastels-on-dark", label: "Pastels on Dar" })
    );
  }, []);
  ...
  return (
    <>
      ...
      <div className="flex md:flex-row flex-col space-x-4 items-start px-4 py-4 mb-[2rem] ">
        <div className="flex flex-col w-full h-full justify-start items-end md:w-[65%] ">
          <CodeEditorWindow
            code={code}
            onChange={onChange}
        ...
        <div className="right-container flex flex-shrink-0 md:w-[30%] flex-col w-[93%]">
